 ...
 *
 * `machines` is a bitfield (MACHINE_CAULDRON / MACHINE_ALEMBIC) of where it can be brewed, so a
 * potion that exists in both tiers is one datum with both bits set - no duplicate recipes. Each
 * machine reads what it needs: the cauldron makes a flat batch from water, the alembic refines a
...
/datum/alch_recipe
	...
	/// Bitfield of machines that can make this recipe (MACHINE_CAULDRON, MACHINE_ALEMBIC).
	var/machines = NONE
	// --- Alembic-only fields, ignored by the cauldron ---
	/// The liquid the alembic consumes as the base: water, or the weaker potion for the "strong" variants.
	var/base_reagent = /datum/reagent/water
	/// Units of base_reagent consumed for one standard brew.
	var/base_reagent_amount = 90
	/// The catalyst that must sit in the alembic - required to react, NEVER consumed.
	var/catalyst = /obj/item/alch/golddust
	/// Units of alembic output per point of ingredient score. Stat potions halve it to 7.5.
...
/// Alembic recipes: the advanced tier, expert-gated, refined from a base liquid + catalyst.
/datum/alch_recipe/alembic
	abstract_type = /datum/alch_recipe/alembic
	machines = MACHINE_ALEMBIC
...
/datum/alch_recipe/proc/generate_html(mob/user)
	...
	if(machines & MACHINE_ALEMBIC)
		var/datum/reagent/base = base_reagent
		html += "Distill at least [base_reagent_amount] drams of [initial(base.name)] in a lit Alembic.<br>"
		if(catalyst)
